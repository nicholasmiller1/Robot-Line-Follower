...
void static SendByte(uint8_t byte){
    ...
    int i;
    for (i = 0; i < 8; i++) {
        // Test the most significant bit of the byte
        // If it's 1, send a 1 on the MOSI line
        if (localByte & 0b10000000) {
            LCD_SEND_1;
        }
        // If it's 0, send a 0 on the MOSI line
        else {
            LCD_SEND_0;
        }

        // Shift the byte left one to access the next bit
        localByte = (localByte << 1) & 0xFF;

        // Toggle SPI clock to signify sending the bit
        LCD_TOGGLE_SCLK;
        LCD_TOGGLE_SCLK;
    }
}
...
void static LCDCommandWrite(uint8_t command){
    ...
    // Set the LCD for sending command bytes
    SET_DC_FOR_COMMAND;
...
void static LCDDataWrite(uint8_t data){
    ...
    // Set the LCD for sending data bytes
    SET_DC_FOR_DATA;
